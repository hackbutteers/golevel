...
var (
	...
	ErrReaderReleased = errors.New("db/table: reader released")
	ErrIterReleased   = errors.New("db/table: iterator released")
)
...
func (e *ErrCorrupted) Error() string {
	return fmt.Sprintf("db/table: corruption on %s (pos=%d): %s", e.Kind, e.Pos, e.Reason)
}
...
func (r *Reader) readBlockCached(bh blockHandle, verifyChecksum, fillCache bool) (*block, util.Releaser, error) {
	if r.cache != nil {
		...
		if ch != nil {
			...
			if !ok {
				ch.Release()
				return nil, nil, errors.New("db/table: inconsistent block type")
			}
			return b, ch, err
		} else if err != nil {
			return nil, nil, err
...
func (r *Reader) newBlockIter(b *block, bReleaser util.Releaser, slice *util.Range, inclLimit bool) *blockIter {
	...
	if slice != nil {
		...
		if bi.offsetStart > bi.offsetLimit {
			bi.sErr(errors.New("db/table: invalid slice range"))
		}
	}
	return bi
}
...
// after use.
//
// Also read Iterator documentation of the db/iterator package.
...
func NewReader(f io.ReaderAt, size int64, fd storage.FileDesc, cache *cache.NamespaceGetter, bpool *util.BufferPool, o *opt.Options) (*Reader, error) {
	if f == nil {
		return nil, errors.New("db/table: nil file")
	}
