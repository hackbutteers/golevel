...
func (e *ErrInternalKeyCorrupted) Error() string {
	return fmt.Sprintf("db: internal key %q corrupted: %s", e.Ikey, e.Reason)
}
...
func makeInternalKey(dst, ukey []byte, seq uint64, kt keyType) internalKey {
	if seq > keyMaxSeq {
		panic("db: invalid sequence number")
	} else if kt > keyTypeVal {
		panic("db: invalid type")
	}
...
func (ik internalKey) assert() {
	if ik == nil {
		panic("db: nil internalKey")
	}
	if len(ik) < 8 {
		panic(fmt.Sprintf("db: internal key %q, len=%d: invalid length", []byte(ik), len(ik)))
	}
}
...
func (ik internalKey) parseNum() (seq uint64, kt keyType) {
	...
	seq, kt = uint64(num>>8), keyType(num&0xff)
	if kt > keyTypeVal {
		panic(fmt.Sprintf("db: internal key %q, len=%d: invalid type %#x", []byte(ik), len(ik), kt))
	}
	return
}
