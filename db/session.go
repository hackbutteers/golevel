...
import (
	...
	"github.com/hackbutteers/groupstore/journal"
...
func (e *ErrManifestCorrupted) Error() string {
	return fmt.Sprintf("db: manifest corrupted (field '%s'): %s", e.Field, e.Reason)
}
...
type session struct {
	...
	stPrevJournalNum int64 // prev journal file number; no longer used; for compatibility with older version of db
	stTempFileNum    int64
...
func (s *session) recover() (err error) {
	defer func() {
		if os.IsNotExist(err) {
			...
			// other files that belong to db. So the DB won't get trashed.
