...
const (
	...
	// If present then db.Recover will drop corrupted 'sorted table'.
	StrictRecovery
...
type Options struct {
	...
	// BlockCacher provides cache algorithm for db 'sorted table' block caching.
	...
	// During reads db will try to find matching filter from
	...
	// unsorted journal.
	//
	// db may held up to two 'memdb' at the same time.
	//
