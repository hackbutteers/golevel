...
func (e *ErrCorrupted) Error() string {
	return fmt.Sprintf("db/journal: block/chunk corrupted: %s (%d bytes)", e.Reason, e.Size)
}
...
var errSkip = errors.New("db/journal: skipped")
...
func (x *singleReader) Read(p []byte) (int, error) {
	r := x.r
	if r.seq != x.seq {
		return 0, errors.New("db/journal: stale reader")
	}
	if x.err != nil {
		return 0, x.err
	}
...
func (w *Writer) fillHeader(last bool) {
	if w.i+headerSize > w.j || w.j > blockSize {
		panic("db/journal: bad writer state")
	}
	if last {
		if w.first {
...
func (w *Writer) Close() error {
	...
	w.err = errors.New("db/journal: closed Writer")
	return nil
}
...
func (x singleWriter) Write(p []byte) (int, error) {
	w := x.w
	if w.seq != x.seq {
		return 0, errors.New("db/journal: stale writer")
	}
	if w.err != nil {
		return 0, w.err
	}
